        ...
        async function fetchSummaryData() {
            ...
            try {
                const urlsToFetch = [
                    BOOKINGS_CSV_URL, CAR_MODEL_CSV_URL, COMMISSION_RATE_CSV_URL,
                    DATA_CSV_URL, FILM_COST_CSV_URL, ...INSTALLER_CSV_URLS
                ];
                
                const texts = await fetchCsvTexts(urlsToFetch);

                if (!texts[0] || !texts[1] || !texts[2]) {
                    throw new Error('ไม่สามารถโหลดข้อมูลหลักได้ครบถ้วน');
                }

                const parsedResults = await Promise.all(texts.map(t => t ? parseCsv(t) : []));
                
                rawBookings = parsedResults[0];
                rawCarModels = parsedResults[1];
                rawCommissionRates = parsedResults[2];
                rawData = parsedResults[3];
                const filmCostRows = parsedResults[4];
                rawInstaller = parsedResults.slice(5).flat();
