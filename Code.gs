...
const SCHEMA = {
  ความเสียหายอื่นๆ: {
    ...
    columns: [
      ...
      'รูปภาพ_1', 'รูปภาพ_2', 'รูปภาพ_3', 'วันที่ลงรายการ'
    ]
  },
  ขนาดกระจก: {
    idPrefix: 'Size',
...
function saveDataRows(sheet, schema, body) {
  ...
  rows.forEach(item => {
    ...
    newValues.push(schema.columns.map(col => {
  ...
  try {
    ...
    if (body.action === 'update' && (body.custId || body.matchPhone || body.recordId || (['PayIn', 'Waranty'].indexOf(sheetName) !== -1 && body.JobID))) {
      ...
      if (targetRowIndex !== -1) {
        ...
        sheet.getRange(targetRowIndex, 1, 1, schema.columns.length).setValues([rowData]);
        return ContentService
